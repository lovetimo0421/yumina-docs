...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | string | 是 | 变量的唯一标识符，AI 通过它来引用变量。在编辑器里可以自己填（会自动转成小写加下划线） |
| `name` | string | 是 | 变量的显示名称，至少 1 个字符。AI 也可以用这个名字来引用变量（引擎会自动做 name -> id 的映射） |
| `type` | `"number"` / `"string"` / `"boolean"` / `"json"` | 是 | 决定了这个变量能存什么、能做什么操作 |
| `defaultValue` | number / string / boolean / object / array | 是 | 游戏开始（或重置）时的初始值。编辑器里叫 **Default Value** |
| `description` | string | 否 | 给自己看的备注。目前编辑器界面没有暴露这个字段，但通过 JSON 导入可以设置 |
| `min` | number | 否 | 仅对 number 类型生效——数值的下限，低于它会自动钳制 |
| `max` | number | 否 | 仅对 number 类型生效——数值的上限，超过它会自动钳制 |
| `category` | 见下方 | 否 | 分类标签，帮助组织和在提示词中分组 |
| `behaviorRules` | string | 否 | 给 AI 的行为指导——告诉它怎么理解和修改这个变量 |
