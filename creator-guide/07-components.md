...
- **内置方法**：编辑器里点 **进入 Studio（Enter Studio）** → 打开 **AI Assistant** 面板 → 用中文告诉它你想要什么
...
在哪设置：**Studio** → **Code View** 面板 → 点 **+** 添加组件 → 写代码或让 AI 写（需要先点 **进入 Studio（Enter Studio）**）
...
编辑器顶部 → 点 **进入 Studio（Enter Studio）**
...
底部显示 **编译状态：OK（Compile Status: OK）** 就成功了。报错就把错误信息发回给 AI 让它修。
