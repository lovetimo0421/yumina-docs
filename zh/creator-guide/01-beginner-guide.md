...
在 Yumina 左侧导航栏点 **创建（Create）** 按钮，你会看到模板选择页面 **创建新世界（Create New）**。这里有五个起点可以选：
...
<!-- 需要截图：模式选择对话框 — Simple / Advanced 两栏 -->

| 模式 | 长什么样 | 适合谁 |
|------|---------|--------|
| **简单（Simple）** | 三个顶层 Tab：首条消息、知识库（内部以引导表单形式展示 AI 角色 / 场景设定 / 语气以及角色列表）、概览。只有最核心的内容会显示 | 刚接触 Yumina、想把脑子里的想法快速变成能玩的世界 |
| **高级（Advanced）** | 完整 9 个区域：首条消息、知识库、变量、行为、自定义 UI、音频、资源、概览、Hub 翻译 | 有明确玩法设计、想用变量/规则/自定义 UI 的创作者 |

**两种模式下的数据是共通的**——简单模式填的东西不会丢，只是换个视图看。高级模式里看到的条目、概览字段都是同一份数据，简单模式只是把"变量、行为、自定义 UI、音频、资源、Hub 翻译"这些高级字段收起来不显示而已。
...
下面介绍的是 **高级模式** 完整的 9 个区域。如果你现在在简单模式里，只能看到首条消息、知识库、概览这三块——看完下面的内容，你自然会理解简单模式隐藏了什么、什么时候该切到高级。
...
左边是导航栏，一共 9 个区域。别被数量吓到，我们一个一个来看 (•̀ᴗ•́)و
...
填多个关键词时，用**英文逗号 `,`** 分隔 —— 比如 `黑市, 走私, 暗巷`。⚠️ 注意必须是英文逗号，中文逗号 `，` 和顿号 `、` 都不会被识别，会导致匹配失败。
...
<!-- 需要截图：编辑器顶部 Variant Tab Bar，两个变体标签（带语言徽章）+ 新建按钮 -->
