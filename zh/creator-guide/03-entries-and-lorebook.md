...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `id` | string | 是 | 唯一标识符，引擎内部用来追踪条目 |
| `name` | string (min 1) | 是 | 条目名称，显示在编辑器里，也用于示例对话中解析角色名 |
| `content` | string | 是 | 条目的正文内容。支持宏替换（如 `{{char}}`、`{{user}}`） |
| `role` | enum | 是 | 条目角色，见上面的表格。可选值：`system`, `character`, `personality`, `scenario`, `lore`, `plot`, `style`, `example`, `greeting`, `custom` |
| `enabled` | boolean | 否 | 是否启用，默认 `true`。禁用的条目完全不参与匹配和注入 |

#### 投送控制

| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `section` | enum | -- (必填) | 条目投送到提示词的哪个区域。四个选项详见下方"四大区域" |
...
| `apiRole` | enum | -- | 编辑器里叫 **发送方式**。`system`（Instruction）= AI 当规则遵守，`user`（User）= AI 以为是玩家说的，`assistant`（AI）= AI 以为是自己说过的。用 `assistant` 可以"预填充"AI 的回复开头 |
...
| `preventRecursion` | boolean | false | 阻止本条目的内容被用于递归扫描。即使本条目被触发了，它的内容也不会去触发其他条目 |
| `excludeRecursion` | boolean | false | 让本条目完全不参与递归扫描。只有第一轮（直接匹配玩家消息）才会考虑它 |
...
| `folderId` | string | 所属文件夹的 ID，用于在编辑器里分组管理条目 |
