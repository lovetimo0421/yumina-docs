...
::: warning 关键词格式：编辑器输入 vs JSON 文件
编辑器里的输入格式和导出的 JSON 文件格式不一样，别搞混：

- **在编辑器里输入**：用**英文逗号 `,`** 分隔，不要加引号 —— 例如 `森林, 树林, 暗影`
- **JSON 文件里存储**：标准 JSON 字符串数组 —— `["森林", "树林", "暗影"]`（双引号是 JSON 语法要求，它本身不参与关键词匹配）

⚠️ **只有英文逗号 `,` 会被识别为分隔符**。中文全角逗号 `，` 和顿号 `、` 都不行 —— 它们会被当成关键词内容的一部分。例如输入 `森林、树林` 会被存成**一个**关键词 `"森林、树林"`，而不是两个，导致匹配失败。
:::
