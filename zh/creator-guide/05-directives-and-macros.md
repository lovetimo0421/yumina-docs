...
上面表里 5 个 `persona_*` 系列的宏都跟 Yumina 的 **玩家人设系统** 有关。这是给玩家用的功能，但你作为创作者需要了解，因为你可以在条目里引用它们。

玩家进入 **设置 → 我的人设（Personas）**，可以创建任意多个"身份"，每个身份有：

- **人设名称（Name）** — 例如"夜语·艾拉拉"、"矮人战士巴托克"
- **头像（Avatar）** — 可选
- **外貌（Appearance）** — 外貌描述
- **性格（Personality）** — 性格特点
- **背景故事（Backstory）** — 身世和经历

同时只能有一个人设处于"使用中"状态。玩家激活哪个，`{{persona_name}}`、`{{persona_appearance}}` 这些宏就会展开成哪个人设的字段。

**`{{user}}` 和 `{{persona_name}}` 的关系**：`{{user}}` 会先看有没有激活的人设——有就用人设名，没有就回落到世界里配置的 `playerName`。所以旧条目用 `{{user}}` 也不用改，玩家换个人设就自动跟着变。

**`{{persona}}` 宏**会把四个字段拼成一段描述：

```
Name: 夜语·艾拉拉
Appearance: 身形修长的精灵法师，银发垂至腰际，左眼有一道贯穿眉骨的疤痕
Personality: 冷淡寡言，但对好奇的事物会突然滔滔不绝
Backstory: 来自已经陷落的月影王国，正在寻找失散的姐姐
```

**创作建议**：在预设组的某个条目里（比如"关于玩家"）放一句 `{{persona}}`，AI 就能持续看到玩家当前扮演的是谁。玩家换人设，AI 马上跟上。不想要全部字段就挑单个宏，比如只想让 AI 知道外貌就用 `{{persona_appearance}}`。

::: tip 导入的 v1 角色卡
从 SillyTavern 导入角色卡时，如果卡上带了 `{{user}}` 宏，Yumina 会自动兼容。玩家任何时候创建并激活人设，旧卡就自动跟上新名字——不需要重新改条目。
:::
