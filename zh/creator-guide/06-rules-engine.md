...
| 编辑器标签 | 内部类型 | 说明 |
|-------------|--------------|-------------|
| 手动触发 | `manual` | 仅在自定义组件调用 `api.executeAction()` 时触发。适用于玩家主动操作，如"打开商店"或"使用技能" |
...
::: warning 定时器系统状态
定时器触发（`timer:fired`）和定时器动作（`start-timer`、`cancel-timer`）可以在编辑器中配置，但客户端定时器运行时尚未完全集成。当前版本中，编辑器中创建的定时器会被保存，但在游戏过程中不会实际倒计时。此功能正在积极开发中。
:::
...
::: warning 尚未可用
此动作可以在编辑器中配置，但定时器目前在游戏运行时不会倒计时。定时器运行时正在积极开发中。
:::
...
::: warning
此定时器示例是设计预览。定时器倒计时在运行时尚未可用——请参阅上方的定时器警告。
:::
