...
**在哪设置：** Studio → Code View → 添加组件 → 在设置里写 `fullScreenComponent: true`
...
| 方法 | 干什么 |
|------|-------|
| `api.playAudio("bgm-battle", { volume: 0.5 })` | 播放音效/音乐 |
| `api.stopAudio("bgm-battle")` | 停止指定音轨 |
...
| 方法 | 干什么 |
|------|-------|
| `api.saveCheckpoint()` | 保存当前进度 |
| `api.loadCheckpoints()` | 加载存档点列表 |
| `api.restoreCheckpoint(id)` | 恢复一个存档点 |
| `api.deleteCheckpoint(id)` | 删除一个存档点 |
