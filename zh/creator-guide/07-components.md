...
你写一个组件替换**整个屏幕**。Yumina 的默认聊天彻底消失——没有消息列表、没有输入框、没有滚动。你的组件就是全部体验。

这就像把剧场拆掉自己盖一个新场馆。每一个像素都由你控制。

你可以从头构建一切，也可以用 `<ChatCanvas />` 嵌入内置聊天，然后在周围添加自己的组件（见下面的 [ChatCanvas 快捷方式](#chatcanvas-快捷方式)）。
...
- **ChatCanvas** — 应用模板专用的内置聊天 UI 组件。`<ChatCanvas />` 提供消息列表、输入框、流式输出、消息编辑、滑动切换、存档点和自动滚动。可选 `messageRenderer` prop 自定义消息样式。
...
| `renderMarkdown` 未定义 | 在消息模板中直接用 `renderMarkdown` prop。在应用模板中用 `useYumina().renderMarkdown`。 |
