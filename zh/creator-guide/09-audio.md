...
| `playMode` | `"loop"` / `"shuffle"` / `"sequential"` | `"loop"` | **loop**: 列表播完从头再来；**shuffle**: 随机打乱顺序播；**sequential**: 按顺序播完一遍后从头再来 |
...
::: tip play/stop 的淡入淡出
AI 指令不支持 `play` 和 `stop` 的淡入淡出参数。如果需要淡入淡出效果，请在 AudioTrack 的定义里设置 `fadeIn` 和 `fadeOut` 字段，或者用 `crossfade` 在曲目之间过渡。
:::
...
**行为系统也能触发音频。** 在行为（Behavior）的 actions 中，有一种 action 类型叫 `play-audio`，字段和 AudioEffect 一样（`trackId`、`action`、`volume`、`fadeDuration`）。这意味着你可以不依赖 AI，纯靠行为引擎来控制音乐——比如"当 hp 降到 20 以下时播放危机音乐"。
