...
**全屏组件（Full Screen Component）** — `surface: "app"` 的组件，接管整个屏幕、默认聊天界面完全隐藏，适合视觉小说 UI、完整游戏界面或全屏自定义体验。参见[渲染器 vs 组件](./07b-renderer-vs-components.md)指南中的**应用组件**。
...
**消息渲染器（Message Renderer）**（在编辑器中称为 **Message Template / 消息模板**）— 用 TSX 编写的自定义组件，替换默认的 Markdown 聊天气泡，实现气泡对话、视觉小说、战斗日志等个性化消息外观。技术上是 `surface: "message"` 的组件。
