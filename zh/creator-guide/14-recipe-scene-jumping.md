...
| 动作类型 | 设置 | 作用 |
|---------|------|------|
| 编辑变量 | `current_route` 设为 `dark` | 记录玩家选了黑暗路线 |
| 切换条目 | 黑暗洞穴世界观 → 启用 | 打开黑暗洞穴的设定 |
| 切换条目 | 阳光草地世界观 → 禁用 | 关掉阳光草地的设定（防止两个同时生效） |

**行为 2：「选择阳光路线」** 同样创建，动作 ID 是 `choose-light`，动作反过来（启用草地、禁用洞穴）。

> **为什么不直接在 TSX 代码里 setVariable 就好了？** 因为 `setVariable` 只能改变量，不能开关条目。行为规则的「切换条目」动作才能在运行时启用/禁用知识库里的条目。所以按钮点击时，我们同时做三件事：`setVariable`（改变量）+ `executeAction`（触发行为规则去开关条目）+ `switchGreeting`（切换开场白）。
...
| 按钮点了但没消失 | 变量没有被正确设置 | 检查聊天页面右侧的变量面板，看 `current_route` 是否变了 |
...
| 选路线后切换世界观 | 创建行为，用「切换条目」动作启用/禁用知识库条目 |
| 切换时播放音效 | 在行为里加「播放音频」动作 |
| 切换时弹出通知 | 在行为里加「通知玩家」动作 |
