...
- **自动检测完成**——当玩家的消息中提到关键词（比如"草药""找到草药"），自动标记任务完成
...
| 行为规则 | `当玩家完成了寻找草药的任务时设为 true。行为规则会通过关键词自动检测，但你也可以在合适的剧情时刻手动标记完成。` | 告诉 AI 这个变量的含义和修改时机 |
...
| 默认值 | `false` | 新会话开始时未完成 |
| 分类 | 标记 | 状态标记 |
| 行为规则 | `当玩家击败了森林狼时设为 true。行为规则会通过关键词自动检测，但你也可以在合适的剧情时刻手动标记完成。` | 告诉 AI 这个变量的含义和修改时机 |
...
| 最小值 | `0` | 防止金币变成负数 |
| 分类 | 资源 | 金币属于资源类变量 |
| 行为规则 | `任务完成时会自动奖励金币。你也可以在剧情中增减金币——例如战斗掉落、交易、或被偷窃。` | 告诉 AI 金币可以在多种场景下变化 |
...
| 字段 | 填什么 | 为什么这样填 |
|------|--------|-------------|
| 触发器类型 | 玩家说了关键词 (`keyword`) | 当玩家发送的消息中出现特定文字时触发 |
| 关键词 | `草药` 或 `找到草药` | 当玩家说"我找到草药了"时匹配 |

> **关键词触发器怎么匹配？** 引擎会检查玩家发送的消息内容——只要消息里**包含**关键词就算匹配。所以玩家说「我找到了草药」会触发，因为内容包含"草药"。如果你还想在 AI 回复中检测关键词，需要另建一条行为，触发器选「AI 说了关键词」(`ai-keyword`)。
...
| 字段 | 填什么 | 为什么这样填 |
|------|--------|-------------|
| 触发器类型 | 玩家说了关键词 (`keyword`) | 和上面一样，玩家关键词触发 |
| 关键词 | `击败` 且 `狼` | 需要同时提到「击败」和「狼」才触发——防止只说"看到一只狼"就误触发 |

> **多个关键词的匹配逻辑。** 当你填多个关键词时，消息里需要同时包含所有关键词才会触发。所以玩家说「我击败了森林狼」会触发（包含"击败"和"狼"），但「我看到一只狼」不会触发（只有"狼"没有"击败"）。
...
4. 发一条消息包含关键词（比如"我找到了草药"）——你的消息里包含"草药"，行为立刻触发，面板自动更新：「寻找草药」变成绿色 ✓，金币变成 30，弹出成就通知
5. 再发一条消息包含关键词（比如"我击败了森林狼"）——你的消息里同时包含"击败"和"狼"，第二个任务也完成，金币增加到 80
...
| 击败狼的任务不触发 | 两个关键词必须同时出现 | 确认你发送的消息里同时包含"击败"和"狼"。如果你写的是"打败了狼"，需要把关键词改成"打败"或者加上"打败"作为备选 |
...
| 检测关键词触发任务完成 | 行为触发器选「玩家说了关键词」(`keyword`)，填入关键词 |
