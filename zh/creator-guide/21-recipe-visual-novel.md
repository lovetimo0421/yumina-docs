...
> 把聊天界面变成一个完整的视觉小说——场景背景、角色立绘、对话框、选项按钮，全都由 AI 通过指令驱动。整个界面由根组件（`index.tsx`）接管，不再用 `<Chat />`，而是自己写全屏布局，必要的地方嵌入 `<MessageInput />` 让玩家发言。纯 Tailwind + 内联样式就能写出来，不需要任何预制组件库。
...
- **场景背景**——AI 通过指令切换背景图片（教室、街道、夜空……），根组件用 `background-image` 全屏显示
- **角色立绘**——AI 通过指令设定当前说话的角色和情绪，根组件用一张 `<img>` 在画面中央显示对应的立绘
- **对话框**——画面底部的半透明对话框，显示角色名和台词。*斜体文字*自动识别为旁白/内心独白，普通文字是角色对话
- **选项按钮**——AI 给出选项时，画面中央叠加一组可点击按钮
- **全屏模式**——根组件直接返回自己的全屏布局，不再嵌套 `<Chat />`，所以没有普通聊天气泡
...
引擎解析这些指令后：
1. `current_bg` 变成 `"classroom_morning.jpg"` → 根组件把 `background-image` 换成教室
2. `current_speaker` 变成 `"小雪"` → 对话框显示角色名「小雪」
3. `speaker_emotion` 变成 `"happy"` → 根组件根据 `speaker + emotion` 拼出立绘路径显示
4. 根组件解析最新一条消息的文本——*斜体*部分作为旁白显示，普通引号对话作为角色台词显示

```
引擎处理流程：
  AI 回复 → 引擎提取指令 → 更新变量 → 根组件读取变量并重绘
    → 背景层：<div style={{ backgroundImage: ... }} />
    → 立绘层：<img src={`/sprites/${speaker}_${emotion}.png`} />
    → 对话框层：区分旁白（灰色斜体）与台词（白色正体）
    → 选项层：show_choices=true 时显示按钮
```
...
1. 打开编辑器 → 左侧边栏 → **资源** 标签页
2. 把图片文件**拖放**到上传区域（或点击浏览选择文件）
3. 上传完成后，每个文件会得到一个 `@asset:` 引用（类似 `@asset:a1b2c3d4-e5f6-7890`）
4. 点击已上传的素材可以**复制引用**

> **`@asset:` 引用是什么？** 它是 Yumina 的内部资源标识符。在根组件 TSX 代码中，`<img src="@asset:xxx" />` 会被自动解析成真实的 CDN 地址。你不需要手动转换——根组件在显示时自动处理。变量里也可以存 `@asset:xxx`，同样会被自动解析。
...
> **为什么首条消息也要写指令？** 因为根组件靠变量来决定显示什么。首条消息的指令会被引擎解析，设好初始的背景和角色状态。如果不写指令，默认值会生效（`default_bg.jpg` + `旁白` + `neutral`），但画面可能不够贴合开场。

---

### 第 5 步：写视觉小说根组件

这是核心步骤。根组件彻底接管整个画面——不再嵌套 `<Chat />`，而是自己画背景、立绘、对话框、选项按钮。玩家的输入通过 `<MessageInput />` 放在画面底部。
...
export default function MyWorld() {
  ...
  return (
    <div style={{
      ...
      {/* ===== 玩家输入层 ===== */}
      {/* 没有 <Chat /> 包裹，直接用 <MessageInput /> 让玩家自由发言 */}
      <div style={{
        position: "absolute",
        bottom: 0,
        left: 0,
...
### 第 6 步：不需要任何开关——根组件就是全屏的

视觉小说天然占满整个屏幕。只要你的 `index.tsx` 的根元素用了 `width: "100%"` + `minHeight: "500px"`（我们上面的代码已经是这样了）或 `100vh`，整个画面就被你的 TSX 接管了，**不需要再开任何"全屏"开关**——因为根组件本来就是世界的 UI 入口。

对照两种写法：
- **普通聊天 + 自定义气泡**：`return <Chat renderBubble={...} />` —— 保留平台的聊天界面，只换气泡样式
- **纯视觉小说（本配方）**：`return <div>...所有 VN 元素...<MessageInput /></div>` —— 不用 `<Chat />`，整个画面是你自己写的

> **想切回聊天怎么办？** 如果你希望保留"普通聊天 + 特殊时刻切视觉小说"的体验，可以在根组件里根据某个变量（比如 `vn_mode`）决定返回 `<Chat />` 还是 VN 全屏布局。AI 通过指令切换这个变量，你就在两种模式之间来回切。
...
| 画面不是全屏 | 根元素没占满可视区域 | 根组件最外层 div 加 `minHeight: "100vh"` 或 `height: "100%"`，并确保父容器有高度 |
...
根组件会按顺序渲染，最终画面显示的是最后一个 `current_speaker` 的立绘。如果你想让每段对话都显示对应角色的立绘，可以在 TSX 里对每个段落单独解析前面最近的 `[current_speaker: set ...]` 指令。
...
| 全屏 VN 体验 | 根组件 `index.tsx` 直接返回 VN 全屏布局（不嵌套 `<Chat />`），底部放 `<MessageInput />` 让玩家发言 |
...
- 一个根组件（完整的 VN 界面：背景 + 立绘 + 对话框 + 选项按钮 + `<MessageInput />`）
