...
我们需要一个变量来追踪当前激活的模式。根组件（Root Component）会读它来决定哪个按钮高亮。
...
| ID | `current_mode` | 行为和根组件用这个 ID 来读写 |
...
| 动作 ID | `mode-comedy` | 根组件里的按钮会调用这个 ID |
...
| 现象 | 可能的原因 | 解决方法 |
|------|-----------|---------|
| 看不到模式按钮 | 根组件代码没保存或有语法错误 | 检查自定义 UI 底部的编译状态，应该显示绿色「OK」 |
...
| 按钮高亮显示当前状态 | 根组件里读变量，用条件样式（`isActive`）控制高亮 |
